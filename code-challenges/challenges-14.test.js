...
const screenForNames = (arr) => {
  ...
  let re = /^(Mr|Mrs|Ms|Dr)\.?(\s*\w*)+$/i;
  
  let target = [...arr.match(re)];
  return target;``
};
...
let biggerThanLuke = (arr) => {
  let names = [];
  for (let c in arr){
    // console.log(`${arr[c].name} mass is ${parseInt(arr[c].mass)}`);
    if (parseInt(arr[c].mass) > 77 ) {
      // console.log(`${arr[c].name}`);
      names.push(arr[c].name);
      // names.join(' - ')
